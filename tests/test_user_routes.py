...
def create_actions(client, sample_project):
    ...
    action_ids = []
    for i in range(1, 4):
        action_data = {"name": f"Action {i}", "id_project": 1}
...
@pytest.mark.parametrize("user_id, expected_status, expected_length", [(1, 200, 1), (999, 200, 0)])
...
@pytest.mark.parametrize(
    "user_id, date_start, date_end, expected_status",
    [
...
def test_get_user_projects_time(
    client, link_user_actions, user_id, date_start, date_end, expected_status
):
