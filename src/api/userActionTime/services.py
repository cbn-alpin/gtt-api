...
def get_user_projects_time_by_id(user_id: int, date_start: str, date_end: str):
    projects_actions_time_tuple = db.session.query(Project, Action, UserActionTime).select_from(UserActionTime) \
    ...
    for project_action_time in projects_actions_time_tuple:
        project_object, action_object, action_time_object = project_action_time
        project = ProjectTimeSchema().dump(project_object)
        action = ActionWithTimeSchema().dump(action_object)
        action_time = ActionTimeSchema().dump(action_time_object)
        ...
        if existing_project:
            ...
            if existing_action:
                existing_action["list_time"].append({"date": action_time["date"], "duration": action_time["duration"]})
            else:
                action["list_time"] = [{"date": action_time["date"], "duration": action_time["duration"]}]
